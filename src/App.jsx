...
function App() {
  ...
  // Sincronização Firestore (Regra 1 e 2)
  useEffect(() => {
    if (!user) return;
  ...
  if (loading) return (
    <div className="h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <p className="text-slate-500 font-medium">A carregar CargoFy...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          {activeTab === 'financeiro' && (
            <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm text-center">
              ...
              <p className="text-slate-500 mb-6">Rentabilidade da frota em tempo real.</p>
      ...
      {/* Modal */}
      <Modal isOpen={modalState.open} onClose={() => setModalState({ open: false, type: '', data: null })} title={modalState.data ? `Editar ${modalState.type}` : `Cadastrar ${modalState.type}`}>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          {/* Outros campos de cadastro omitidos para brevidade, mas mantendo a lógica de InputField */}
          {(modalState.type === 'clientes' || modalState.type === 'motoristas' || modalState.type === 'veiculos') && (
            <div className="grid grid-cols-1 gap-4">
              <InputField label="Nome / Referência" name={modalState.type === 'veiculos' ? 'placa' : 'nome'} defaultValue={modalState.data?.nome || modalState.data?.placa} />
              <InputField label="Informação Adicional" name={modalState.type === 'clientes' ? 'cnpj' : 'cpf'} defaultValue={modalState.data?.cnpj || modalState.data?.cpf} />
            </div>
          )}
          <div className="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold">Salvar Dados</button>
          </div>
        </form>
      </Modal>
    </div>
...
function InputField({ label, name, type = "text", options = [], defaultValue }) {
  const css = "w-full px-3 py-2 bg-slate-50 border rounded-lg text-sm mt-1 outline-none focus:ring-2 focus:ring-blue-500 transition-all";
  return (
    <div>
